...
class QualityAssessment(BaseModel):
    ...
    counseling_techniques_score: float = Field(
        ..., ge=0, le=10, description="咨询技巧评分(0-10)"
    )
    final_result: str = Field(..., description="最终结果描述")
    issue_consistency: bool = Field(..., description="最终结果是否与初始设置一致")
    overall_quality_score: float = Field(
        ..., ge=0, le=10, description="总体质量评分(0-10)"
    )
...
class ConversationData(BaseModel):
    ...
    current_counselor_state: CounselorState = Field(
        default=CounselorState.INTRODUCTION, description="当前咨询师状态"
    )

...
from llm_agent.base import (
    ChatBot,
    Agent,
    RiskAssessmentMixin,
    convert_history_for_student,
    convert_history_for_counselor,
)
...
from llm_agent.flow_control import FlowControlAgent
from llm_agent.quality_assess import QualityAssessmentAgent
...
class TestHistoryConversionFunctions:
    ...
    def sample_history(self):
        ...
        return [
            ...
            ConversationMessage(
                role="counselor", content="我能理解你的感受", round_number=1
            ),
...
class TestFlowControlAgent:
    ...
    def test_get_state_transition_recommendation(self, flow_agent):
        """测试状态转换建议"""
        # 不需要转换
        no_transition = {"state_transition": {"need_transition": False}}
...
class TestQualityAssessmentAgent:
    ...
    def sample_background_info(self):
        ...
        return BackgroundInfo(student_info=student_info, counselor_info=counselor_info)
    ...
    def test_analyze_counseling_trajectory(self, quality_agent):
        ...
        sample_history = [
            ConversationMessage(
                role="counselor", content="欢迎", state="introduction", round_number=1
            ),
            ConversationMessage(role="student", content="谢谢", round_number=1),
            ConversationMessage(
                role="counselor", content="探索", state="exploration", round_number=2
            ),
        ]
    ...
    def test_calculate_quantitative_metrics(self, quality_agent):
        ...
        conversation_history = [
            ...
            ConversationMessage(
                role="counselor", content="这是一个较长的咨询师回复？", round_number=1
            ),
            ConversationMessage(
                role="student",
                content="另一个学生消息",
        ...
        metrics = quality_agent._calculate_quantitative_metrics(
            conversation_history, {}
        )
...
class TestIntegrationScenarios:
    ...
    async def test_complete_conversation_flow_mock(self):
        ...
        with patch.object(
            bg_agent, "_safe_json_parse", return_value=mock_background_response
        ):
            background, initial_question = await bg_agent.generate_background()
    ...
    def test_risk_assessment_integration(self):
        ...
        risky_conversation = [
            ConversationMessage(
                role="student", content="我觉得活着没意思", round_number=1
            ),
            ConversationMessage(
                role="counselor", content="能详细说说吗？", round_number=1
            ),
            ConversationMessage(
                role="student", content="我想要结束这一切", round_number=2
            ),
        ]

...
from agents.base import ChatBot, RiskAssessmentMixin, convert_history_for_student
from models import (
    StudentBackground, EmotionState, ConversationMessage
)
from constants import EMOTION_TRANSITIONS, PSYCHOLOGICAL_ISSUES_DATA
...
class StudentBot(ChatBot, RiskAssessmentMixin):
    ...
    def _init_config(self, **kwargs):
        ...
        # 学生背景信息
        self.student_background: Optional[StudentBackground] = kwargs.get('student_background')
    ...
    def _determine_initial_emotion(self) -> EmotionState:
        ...
        issue = self.student_background.current_psychological_issue
        
        # 根据不同心理问题设定初始情绪
        emotion_mapping = {
            "academic_anxiety": EmotionState.ANXIOUS,
            "social_phobia": EmotionState.ANXIOUS,
            "depression": EmotionState.DEPRESSED,
            "procrastination": EmotionState.CONFUSED,
            "ocd_symptoms": EmotionState.ANXIOUS,
            "adaptation_issues": EmotionState.CONFUSED,
            "relationship_issues": EmotionState.DEPRESSED,
            "family_conflicts": EmotionState.ANGRY,
            "identity_confusion": EmotionState.CONFUSED,
            "sleep_problems": EmotionState.ANXIOUS
        }
        
        return emotion_mapping.get(issue.value, EmotionState.ANXIOUS)
...
"""
    ...
    async def generate_student_response(self, conversation_history: List[ConversationMessage]) -> str:
        """
        生成学生的回复
        ...
        self.current_round = len([msg for msg in conversation_history if msg.role == "student"]) + 1
    ...
    async def _adjust_emotion_state(self, counselor_response: str):
        ...
        if any(indicator in response_lower for indicator in positive_indicators):
            # 倾向于积极情绪转换
            positive_emotions = [EmotionState.CALM, EmotionState.TRUSTING, EmotionState.HOPEFUL]
            available_positive = [e for e in current_transitions if e in positive_emotions]
            ...
            if random.random() < self.resistance_level:
                ...
                available_negative = [e for e in current_transitions if e in negative_emotions]
    ...
    def should_reveal_information(self, information_level: str) -> bool:
        ...
        return self.trust_level >= threshold and random.random() < (self.trust_level + self.openness_level) / 2

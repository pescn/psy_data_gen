...
请只返回JSON格式，不要包含其他解释。"""
    ...
    def _build_issues_reference(self) -> str:
        ...
        for issue, data in PSYCHOLOGICAL_ISSUES_DATA.items():
            ...
            reference += (
                f"学生表达方式示例：{', '.join(data['student_expressions'][:3])}\n"
            )
    ...
    def _parse_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "initial_question" not in parsed_data:
                ...
                # 查找Q0内容
                initial_question = self._extract_initial_question_from_symptom(
                    symptom_desc
                )
                if initial_question:
            ...
            student_required = [
                ...
                "hidden_personal_info",
                "current_psychological_issue",
                "symptom_description",
            ]
            ...
            raise ValueError(
                f"Failed to parse background generation response: {str(e)}"
            )
    ...
    async def generate_background(
        ...
        try:
            ...
            for issue in PsychologicalIssue:
                if (
                    issue.value == issue_str
                    or PSYCHOLOGICAL_ISSUES_DATA[issue]["name"] == issue_str
                ):
            ...
            for approach in TherapyApproach:
                if (
                    approach.value == approach_str
                    or THERAPY_APPROACHES_DATA[approach]["name"] == approach_str
                ):
    ...
    def _calculate_completeness_score(self, background: BackgroundInfo) -> float:
        ...
        # 学生信息检查
        if len(student.family_background) >= 100:
            score += 1
        if len(student.psychological_profile) >= 150:
            score += 1
        if len(student.hidden_personal_info) >= 200:
            score += 1
        if len(student.symptom_description) >= 200:
            score += 1
        if len(student.personality_traits) >= 3:
            score += 1

        # 咨询师信息检查
        if len(counselor.communication_style) >= 100:
            score += 1
        if len(counselor.specialization) >= 2:
            score += 1
        if 3 <= counselor.experience_years <= 15:
            score += 1

        # 整体一致性检查
        if student.major in COMMON_MAJORS:
            score += 1
        if student.grade in COMMON_GRADES:
            score += 1
